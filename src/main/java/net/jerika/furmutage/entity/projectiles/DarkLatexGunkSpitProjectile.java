...
/**
 * Projectile that spreads dark latex layer on impact (2-block radius).
 */
public class DarkLatexGunkSpitProjectile extends ThrowableItemProjectile {

    private static final int SPLAT_RADIUS = 2;
    private static final int SPLAT_RADIUS_SQ = SPLAT_RADIUS * SPLAT_RADIUS;

    public DarkLatexGunkSpitProjectile(EntityType<? extends DarkLatexGunkSpitProjectile> entityType, Level level) {
        super(entityType, level);
    }

    public DarkLatexGunkSpitProjectile(Level level, LivingEntity shooter) {
        super(ModEntities.DARK_LATEX_GUNK_SPIT_PROJECTILE.get(), shooter, level);
    }

    public DarkLatexGunkSpitProjectile(Level level, double x, double y, double z) {
        super(ModEntities.DARK_LATEX_GUNK_SPIT_PROJECTILE.get(), x, y, z, level);
    }

    @Override
    protected Item getDefaultItem() {
        return ModItems.DARK_LATEX_GUNK_SPIT.get();
    }
